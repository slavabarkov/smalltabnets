# This code is a derivative work of the TabM code which is licensed Apache-2.0.
...
def default_zero_weight_decay_condition(
    ...
    # In Python 3.9, we can't use Union with isinstance() directly
    # Instead, we need to check each type individually
    return parameter_name.endswith("bias") or (
        isinstance(module, nn.BatchNorm1d)
        or isinstance(module, nn.LayerNorm)
        or isinstance(module, nn.InstanceNorm1d)
        or isinstance(module, rtdl_num_embeddings.LinearEmbeddings)
        or isinstance(module, rtdl_num_embeddings.LinearReLUEmbeddings)
        or isinstance(module, _Periodic)
    )
