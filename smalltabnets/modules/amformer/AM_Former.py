                ...
                nn.ModuleList(
                    [
                        ...
                        (
                            ...
                            if token_descent
                            else Attention(heads=heads, dim=dim, dropout=attn_dropout)
                        ),
                        nn.Linear(2 * (groups[i] + flag), groups[i] + flag),
                        (
                            nn.Linear(token_num + flag, groups[i] + flag)
                            if token_descent
                            else nn.Identity()
                        ),
    ...
    def forward(self, x):
        ...
        for toprod, tosum, down, downx, ff in self.layers:
            ...
            if self.use_prod:
                prod = toprod(x)
                attn_out = down(
                    torch.cat([attn_out, prod], dim=1).transpose(2, 1)
                ).transpose(2, 1)
    ...
    def __init__(
        ...
        if self.num_unique_categories > 0:
            categories_offset = F.pad(
                torch.tensor(list(args.categories)), (1, 0), value=num_special_tokens
            )
        ...
        self.to_logits = nn.Sequential(
            nn.LayerNorm(dim), nn.ReLU(), nn.Linear(dim, out)
        )
