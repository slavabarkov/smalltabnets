...
class ProdAttention(nn.Module):
    def __init__(
        ...
        self.out = nn.Sequential(nn.Linear(dim, dim), nn.Dropout(0.2))
...
class MemoryBlock(nn.Module):
    def __init__(
        ...
        self.out = nn.Sequential(nn.Linear(dim, dim), nn.Dropout(attn_dropout))
        ...
        if cluster:
            if target_mode == "mix":
                ...
                self.to_target = nn.Linear(
                    groups + token_num + int(use_cls_token), groups + int(use_cls_token)
                )
            else:
                self.target_token = nn.Parameter(
                    torch.rand([groups + int(use_cls_token), dim])
                )
    ...
    def forward(self, x):
        ...
        if self.num_per_group == -1:
            ...
            value, idx_original = torch.topk(
                attn, dim=-1, k=self.num_per_group
            )  # bs head group num_per_group
            ...
            # ???????????????????????????????
            # x = xx_.sum(dim=-2)
            x = self.gather_layer(xx_.reshape(b * h, -1, d // h)).reshape(
                b, h, -1, d // h
            )
