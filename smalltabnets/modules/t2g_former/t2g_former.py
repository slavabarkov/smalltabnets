# code from https://github.com/jyansir/t2g-former, licensed MIT
    ...
    return (
        ...
        if name == "reglu"
            ...
            if name == "geglu"
                ...
                torch.sigmoid
                if name == "sigmoid"
                else tanglu if name == "tanglu" else getattr(F, name)
            )
        )
    )
...
class MultiheadGEAttention(nn.Module):
    ...
    def __init__(
        ...
        d_col = math.ceil(
            2 * math.log2(self.n_cols)
        )  # dim for column header embedding -> d_header += d
    ...
    def _reshape(self, x: Tensor) -> Tensor:
        ...
        return x.reshape(batch_size, n_tokens, self.n_heads, d_head).transpose(1, 2)
    ...
    def _get_topology(self, top_score, elewise_func=torch.sigmoid):
        ...
        """
        adj_probs = elewise_func(
            top_score + self.bias
        )  # choose `sigmoid` as element-wise activation (sigma1)
        if self.nsi:
            adj_probs = self._no_self_interaction(adj_probs)  # apply `nsi` function
        adj_probs = self._prune_to_readout(
            adj_probs
        )  # cut edges from features to [Readout]
    ...
    def forward(
        ...
        weight_score = (
            f_head
            @ torch.diag_embed(self.rel_emb)
            @ f_tail.transpose(-1, -2)
            / math.sqrt(d_head_tail)
        )

        col_emb_head = F.normalize(
            self.col_head, p=2, dim=-1
        )  # L2 normalized column embeddings
        ...
        fr_graph = comp_func(
            weight_score + adj_mask, dim=-1
        )  # choose `softmax` as competitive function

        if self.dropout is not None:
            fr_graph = self.dropout(fr_graph)
        x = fr_graph @ self._reshape(f_v)
        x = x.transpose(1, 2).reshape(batch_size, n_head_nodes, self.n_heads * d_value)
        if self.W_out is not None:
            x = self.W_out(x)
...
class T2GFormer(nn.Module):
    ...
    def forward(
        self, x_num: Tensor, x_cat: ty.Optional[Tensor], return_fr: bool = False
    ) -> Tensor:
